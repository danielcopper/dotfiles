...
- **After a pull that touched this repo, run `mise run dotfiles-relink`.** It re-links the `claude` package and restores that write protection. Both are needed: a new hook script has no symlink yet while `settings.json` already wires it (which breaks *every* Bash call until stow runs), and git resets the file's mode to 644 whenever it rewrites it. Pass package names to relink others: `mise run dotfiles-relink nvim tmux`. Context-specific guidance is kept out of the always-loaded `claude/.claude/CLAUDE.md` and lives in skills instead (`worktree`, `azure-devops-boards`, `local-sql-server`, …).
